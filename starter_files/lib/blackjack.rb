...
class Game
  ...
  attr_accessor :player_hand, :dealer_hand, :deck, :player_value, :dealer_value, :win_tracker, :loss_tracker, :bank, :bet
  ...
  def player_phase
    ...
    if @player_value == 21
      ...
      puts "Would you like a hit? Press (y / n)"
  ...
  def hit(active_hand)
    if active_hand == @player_hand
      ...
      @player_value = @player_hand.reduce(0) {|sum, value| sum + value.value}
      if @player_value <= 21
        num = @player_hand.length
        puts "You ended up drawing a #{@player_hand[num - 1]}"
      ...
      dcard = @dealer_hand.length
      @dealer_hand << @deck.draw_card
      @dealer_value = @dealer_hand.reduce(0) {|sum, value| sum + value.value}
      puts "The dealer drew a #{@dealer_hand[dcard - 1]}"
      puts "The dealers new value is #{@dealer_value}"
    end
  end
